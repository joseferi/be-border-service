...
func LimitDefaultValue(origin int64) int64 {
	if origin < 1 {
		return constants.PagingLimitDefaultValue
	}

	if origin > constants.PagingMaxLimit {
		return constants.PagingMaxLimit
	}
...
func PageDefaultValue(origin int64) int64 {
	if origin < 1 {
		return constants.PagingPageDefaultValue
	}
...
func OffsetDefaultValue(page int64, limit int64) int64 {
	if page < 1 {
		return constants.PagingOffsetDefaultValue
	}
